...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    auto findOne = dict.find(newWord1);
	auto findTwo = dict.find(newWord2);


     if(findOne == dict.end() || findTwo == dict.end()){
      return false;
    }
    
    if(findOne->second.size() != findTwo->second.size()){
      return false;
    }
    
    if (findOne->second != findTwo->second){
        return false;
    }
