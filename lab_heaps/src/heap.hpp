...
return currentIdx <= (_elems.size() - 1) / 2;
}
...
void heap<T, Compare>::heapifyDown(size_t currentIdx)
{
    ...
    if(higherPriority(_elems[max] , _elems[currentIdx])){
        swap(_elems[max], _elems[currentIdx]); 
        heapifyDown(max); 
    }
...
void heap<T, Compare>::heapifyUp(size_t currentIdx)
{
    ...
    if (higherPriority(_elems[currentIdx], _elems[parent_])) {
        swap(_elems[currentIdx], _elems[parent_]);
        heapifyUp(parent_);
    }
}
...
heap<T, Compare>::heap(const std::vector<T>& elems) 
{
    ...
    size_t curr = parent(_elems.size() - 1);
    while (curr >= root()) {
        heapifyDown(curr);
        curr--;
    }
