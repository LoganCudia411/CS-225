...
{
    TreeNode* temp = NULL; 
    if(singleQueue.size() < 1 && mergeQueue.size() < 1){
        return NULL; 
    }
    ...
    auto merg_freq = mergeQueue.front()->freq.getFrequency(); 
    auto sing_freq = singleQueue.front()->freq.getFrequency(); 

    if(merg_freq < sing_freq){
        temp = mergeQueue.front(); 
        mergeQueue.pop(); 
        return temp; 
    }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
    TreeNode* small = NULL; 
    TreeNode* nextSmall = NULL; 
    for(unsigned i = 0; i < frequencies.size(); i++){
        TreeNode * n = new TreeNode(frequencies[i]); 
        singleQueue.push(n); 
    }
    auto sumSize = singleQueue.size() + mergeQueue.size(); 

    while(sumSize >= 2){
        small = removeSmallest(singleQueue, mergeQueue);
        nextSmall = removeSmallest(singleQueue, mergeQueue); 

         int f = (small->freq).getFrequency();
        int fNext = (nextSmall->freq).getFrequency();

        TreeNode* combine = new TreeNode(fNext + f);
        combine->right = nextSmall;
        combine->left = small;
        
        mergeQueue.push(combine);

        sumSize = singleQueue.size() + mergeQueue.size();
    }
    ...
    while (bfile.hasBits()) {
        ...
        if(bfile.getNextBit()){
         ...
         if((!current->left) && (!current->right)){
            auto shift = (current->freq).getCharacter(); 
            ss<<shift; 
            current = root_; 
        }
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
    ...
    if((!current->right) && (!current->left)){
        bfile.writeBit(1); 
        auto byte = (current->freq).getCharacter(); 
        bfile.writeByte(byte); 
    } else{
        bfile.writeBit(0); 
