...
class KDTree
{
  ...
  public:
     ...
     */
    KDTreeNode* buildTree(vector <Point<Dim>> & vector, int curDim, int left, int right);
    ...
    void _copy(KDTreeNode* Tree1, KDTreeNode* Tree2); 
    ...
    KDTreeNode * opHelper(KDTreeNode* node);
    /**
     ...
     */
     Point<Dim> neightborHelper(const Point<Dim> & query, KDTreeNode* curr, int dim) const;
