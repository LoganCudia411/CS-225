...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if(first_larger_idx < subroot->elements.size()){ //check if valid size
        if(subroot->elements[first_larger_idx].key == key){ //then check if key 
            return subroot->elements[first_larger_idx].value; //we are done and return the coreesponding value 
        }
    }
     ...
     */
     //if a leaf, return the default 
    if(subroot->is_leaf){
         return V();
    }
    
    return find(subroot->children[first_larger_idx] , key); //recurse
}
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    //if the element is already in tree
    if((first_larger_idx < subroot->elements.size()) && (subroot->elements[first_larger_idx] == pair)){
        return; 
    }

    BTreeNode *succ = subroot->children[first_larger_idx]; 
    insert(succ,pair); 
    //check if child is too large and needs to be split
    if(succ->elements.size() >= order){
